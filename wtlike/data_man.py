...
# %% auto 0
__all__ = ['GTI', 'get_ft1_data', 'get_ft2_info', 'FermiData', 'check_data', 'update_data', 'week_livetime_map',
           'DataView', 'project_to_map', 'PixelView']
...
def week_livetime_map(week, config=None, nside=None, sigma=0, frame='galactic'):
    """Return a HEALPix map of spacecraft livetime for a single mission week.

    Parameters
    ----------
    week : int
        Mission week number (e.g., 9+).
    config : Config or None
        Optional ``Config`` instance. Uses default ``Config()`` if omitted.
    nside : int or None
        HEALPix nside for the output map. Defaults to ``config.nside``.
    sigma : float
        Optional Gaussian smoothing width in degrees.
    frame : str
        Output coordinate frame for the map. One of ``'galactic'`` (default)
        or ``'fk5'``.

    Returns
    -------
    np.ndarray
        RING-ordered HEALPix map with per-pixel integrated livetime (seconds).
    """
    import pickle
    from astropy.coordinates import SkyCoord

    cfg = config or Config()
    nside = nside or cfg.nside
    npix = 12 * nside**2

    week_files = get_week_files(cfg, (week, week))
    if len(week_files) == 0:
        raise ValueError(f'No week file found for week {week}.')

    # Keep only exact week matches in case neighboring weeks are included.
    week_files = [wf for wf in week_files if int(wf.name[-7:-4]) == int(week)]
    if len(week_files) == 0:
        raise ValueError(f'No exact week file found for week {week}.')

    with open(week_files[0], 'rb') as inp:
        week_data = pickle.load(inp)

    sc_data = week_data.get('sc_data', None)
    if sc_data is None or len(sc_data.get('livetime', [])) == 0:
        return np.zeros(npix, dtype=np.float32)

    if frame.lower() == 'galactic':
        coords = SkyCoord(sc_data['ra_scz'], sc_data['dec_scz'], unit='deg', frame='fk5').galactic
        lon, lat = coords.l.deg, coords.b.deg
    elif frame.lower() == 'fk5':
        lon, lat = sc_data['ra_scz'], sc_data['dec_scz']
    else:
        raise ValueError("frame must be 'galactic' or 'fk5'")

    ipix = healpy.ang2pix(nside, lon, lat, lonlat=True)
    ltmap, _ = np.histogram(ipix, bins=np.arange(npix + 1), weights=sc_data['livetime'])

    if sigma > 0:
        ltmap = healpy.smoothing(ltmap, np.radians(sigma))

    return np.asarray(ltmap, dtype=np.float32)
